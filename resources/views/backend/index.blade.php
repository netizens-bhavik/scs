                                            ...
                                            <div class="row">
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <select class="form-control form-select" name="country_id"
                                                            ...
                                                            @foreach ($all_country as $branch)
                                                                <option value="{{ $branch['id'] }}">{{ $branch['country_name'] }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                        ...
                                                        <select class="form-control form-select" name="user_id"
                                                            ...
                                                            @foreach ($all_users as $user)
                                                                <option value="{{ $user['id'] }}">
                                                                    {{ ucwords($user['role_name']) }}-{{ ucwords($user['country_name']) }}-{{ ucwords($user['name']) }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                ...
                                                <div class="col-sm-4">
                                                    <div class="form-group">
                                                        ...
                                                        <div>
                                                            <button type="button" class="btn btn-primary fltr-btn" id="fltr-btn"
                                                                title="Filter">Filter
                                                            </button>
                                                        </div>
                                                    </div>
    ...
    <script>
                ...
                $.ajax({
                    ...
                    beforeSend: function() {
                        $('#datatable-space').html(
                            '<div class="text-center"><img src="{{ asset('public/assets/images/scload.gif') }}" alt="loader"><h3>Loading...</h3></div>'
                        );
                    },
        ...
        function get_followup_count(country_id = null, user_id = null) {
            $.ajax({
                ...
                beforeSend: function() {
                    $('#today-followups').html(
                        '<img src="{{ asset('public/assets/images/stats_load.gif') }}" alt="loader" width="22px" height="22px">'
                    );
                    $('#tomorrow-followups').html(
                        '<img src="{{ asset('public/assets/images/stats_load.gif') }}" alt="loader" width="22px" height="22px">'
                    );
                    $('#pending-followups').html(
                        '<img src="{{ asset('public/assets/images/stats_load.gif') }}" alt="loader" width="22px" height="22px">'
                    );
                    $('#future-followups').html(
                        '<img src="{{ asset('public/assets/images/stats_load.gif') }}" alt="loader" width="22px" height="22px">'
                    );
                },
                success: function(data) {
        ...
        function status_clients() {



            $(document).on('click', '.country_wise_clients', function() {
                var country_name = $(this).prev().find('h6').text();
                var country_id = $('#dashboard_country_id').val() || 0;
                var user_id = $('#dashboard_user_id').val() || 0;

                $.ajax({
                    url: "{{ url('country_wise_clients_list') }}",
                    method: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                    ...
                    success: function(data) {
                        $('#clientModalBox').html('');
                        $('#clientModalBox').html(data.html);
                        $('#viewClientsModal').modal('show');
                    }
                });
            });

            // $(document).on('click', '.job_status_clients', function() {
            //     // var job_status = $(this).prev().find('h6').text();
            //     // alert(job_status);
            // });

            $(document).on('click', '.mom_status_clients', function() {
                var mom_status = $(this).prev().find('h6').text();
                var country_id = $('#dashboard_country_id').val() ?? 0;
                var user_id = $('#dashboard_user_id').val() || 0;

                $.ajax({
                    url: "{{ url('mom_status_clients_list') }}",
                    method: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
